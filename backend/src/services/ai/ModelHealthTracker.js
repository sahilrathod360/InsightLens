...
// Tracks real telemetry (latency, success/failure counts, 402/429 status, cooldowns)
...
class ModelHealthTracker {
  ...
  recordFailure(modelId, errorType = 'UNKNOWN', latencyMs = 0, reason = '') {
    ...
    if (errorType === 'HTTP_402' || reason.includes('402') || reason.includes('credits')) {
      s.http402Count++;
      // Cooldown 15 minutes for credit exhaustion
      s.cooldownUntil = now + (15 * 60 * 1000);
      console.warn(`[HealthTracker] Model ${modelId} placed on 15-min cooldown due to credit exhaustion (HTTP 402)`);
