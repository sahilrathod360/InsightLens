...
export const analyzeArtifact = async (req, res, next) => {
  try {
    ...
    const reportId = report.id || `RPT-${Date.now()}-${Math.floor(Math.random() * 10000)}`;
    const now = Date.now();
    ...
    // Assign report ID before persistence
    report.id = reportId;
    ...
    try {
      ...
      console.log(`[AnalysisController] Report successfully persisted to PostgreSQL. ID: ${persistedReportId}`);
    } catch (dbErr) {
      console.error('[AnalysisController] PostgreSQL Report Persistence Failure:', dbErr);
      return res.status(500).json({
        success: false,
    ...
    console.log(`[Backend] Returned report successfully with ID: ${persistedReportId}`);
